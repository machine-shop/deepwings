...
#species_6cells = ['MAWIspB', 'acuminatum', 'bimaculata', 'calcarata', 'coloradensis', 
#                  'seocolis',	'impatiens',	'leucozonium','	lignaria','	nymphaerum',	'pilosum',
#                  'pusilla','ribifloris','	rohweri','	texana','	texanus','	virescens','	zephyrum',
#                  '	zonulum']
#
#species_7cells = ['MAWIspB	', 'acuminatum', 'bimaculata', 	'calcarata',
#                  'coloradensis', '	coriaceum	', 'griseocolis', '	impatiens	', 'leucozonium', 
#                  'nymphareum','	lignaria', '	nymphaerum', '	pilosum	', 'pusilla', '	ribifloris', 
#                  '	rohweri', '	sericeus', '	texana', 'texanus', 'virescens', 'zephyrum',
#                  'zonulum']
#																																																																																																																													MAWIspB	acuminatum	bimaculata	calcarata	coloradensis	coriaceum	griseocolis	impatiens	leucozonium	lignaria	nymphaerum	pilosum	pusilla	ribifloris	rohweri	sericeus	texana	texanus	virescens	zephyrum	zonulum
#
...
def create_csv(folder_path, category, nb_cells):
    ...
    for name in names:
        if category == 'genus':
            ...
            category_name = (name.split(' ')[1] + '_' + name.split(' ')[2]).lower()
    ...
    with open(new_csv_name, 'a') as new_csv_file:
        ...
        for row in data:
            ...
            if category == 'genus':
                ...
                category_name = (row[0].split(' ')[1] + '_' + row[0].split(' ')[2]).lower()
...
def ANN_classifier(input_dim, output_dim):
    print('input_dim : %i  output_dim: %i' %(input_dim, output_dim))
    classifier = Sequential()
    classifier.add(Dense(output_dim = input_dim, init = 'uniform', activation = 'relu', input_dim = input_dim))
    classifier.add(Dropout(p = 0.55)) #avoiding overfitting
    classifier.add(Dense(output_dim = output_dim, init = 'uniform', activation = 'softmax'))
...
def train(category):
    ...
    for nb_cells in [6, 7]:
        ...
        Y = data[:, -nb_categories:] # remove last column for Dummy variables trap
        ...
        classifier = ANN_classifier(input_dim=X.shape[1], output_dim=Y.shape[1])
        
        #compiling the ANN
        classifier.compile(optimizer = 'adam', loss = 'categorical_crossentropy', metrics = ['accuracy'])
        
        history = classifier.fit(X, Y, batch_size = 40, nb_epoch = 300, validation_split = 0.2, shuffle=True)
        ...
        classifier.save(prefix + category +'_' +str(nb_cells) +'cells_ann_model.h5')
...
def predict(category, plot, n_descriptors, nb_pred=3):
    ...
    if not ( bool_6cells or bool_7cells or bool_inv):
        print('ERROR: no CSV files found, try extracting features before predicting')
        return 0
    ...
    for nb_cells in [6, 7]:
        ...
        scaler_name = 'training/scaler_'+str(nb_cells) + 'cells.save'
        processed_csv = 'training/' + category + '_data_' + str(nb_cells)+'cells.csv'
        ...
        prefix = 'method_features_extraction/classifiers/models_ann/'
        classifier = load_model( prefix + category + '_' + str(nb_cells) + 'cells_ann_model.h5')
    ...
    print("Prediction successful, results in 'deepwings/prediction/prediction_ann.csv' ")
